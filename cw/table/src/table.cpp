	...
	bool upper_bound_inclusive)
{
    ...
    {
	    throw std::logic_error("upper bound less than lower");
    }
...
table table::load_data_from_filesystem(std::string const &filename)
{
    ...
    while (std::getline(input_file, line))
    {
        ...
        if (seg_list.size() != 4)
        {
            throw std::logic_error("unexpected string in file");
        }
...
void table::insert_table_to_filesystem(const std::filesystem::path &path)
{
    ...
    while (tbl_it != tbl_end)
    {
        ...
        auto out_str = std::to_string(string_pool::add_string(target_key)) + "#" + std::to_string(target_value.get_id()) + "#" + target_value.get_name() + "#" + target_value.get_surname() + "|";
        throw_if_exceeds_length_limit(out_str);
        length_alignment(out_str);
